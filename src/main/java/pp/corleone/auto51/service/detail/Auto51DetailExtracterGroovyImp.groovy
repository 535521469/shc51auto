...
@Component("auto51DetailExtracter")
class Auto51DetailExtracterGroovyImp implements Auto51DetailExtracter {
	...
	protected void fillPrice(Document doc,Auto51CarInfo auto51CarInfo){
		try{
			String price =
					doc.select("span.car_color_in").first().select("em").first().text();
			price = price+"万";
	...
	protected void fillLicenseDateAndRoadHaul(Document doc,Auto51CarInfo auto51CarInfo){
		try{
	...
	protected void fillDisplacementAndGearbox(Document doc,Auto51CarInfo auto51CarInfo){
		try{
			Elements infoElements  =doc.select("p.car_info1");
	...
	protected void fillManufacturerAndBrand(Document doc,Auto51CarInfo auto51CarInfo){
		try{
			Element paramElement = doc.select("dd.car_dsc_1>dl.params").first();

			String manufacturer = paramElement.select("dd").first().select("b").first().text();
			auto51CarInfo.setManufacturer(manufacturer);

			String brand = doc.select("div.daohang>a[href^=http://www.51auto.com/search/s-]").first().text();
